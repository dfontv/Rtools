...
# It has been tested with versions 3.1 and 3.4
...
# 1. minor edits to work with version 3.1
...
wd_output = ".../output" # this folder needs to be created
...
if (run.step.1.2 == T){
  ...
  for ( f in 1:length(files) ){ # for every spold file
    ...
    if (is.null(classificationId)) {classificationId = NA} # if there is no classification (only in v3.1)
...
for ( f in 1:nrow(flow.Data.df) ){ # for each intermediate/elementary flow
  ...
  if ( flow.Data.df$Exchange[f] == 'intermediate' && flow.Data.df$Group[f] == 'output' &&
    ...
    if ( !is.na(activityLinkId) && value != 0 ){
      ...
      if (length(row.index) == 0){ row.index = NA }
    }
...
intermediate.exchanges = flow.Data.df[ (flow.Data.df$Exchange == 'intermediate' & 
                                          ...
                                          is.na(flow.Data.df$col.index) == F),]
